...
export function extractProductName(text: string, structuredLines?: readonly OcrLine[]): string | null {
  const spatialSuggestion = structuredLines && structuredLines.length > 0
    ? extractFromStructuredLines(structuredLines)
    : null
  if (spatialSuggestion) return spatialSuggestion
  ...
  return selectProductName(candidates)
}

function extractFromStructuredLines(lines: readonly OcrLine[]): string | null {
  ...
  const heights = orderedLines
    .map(({ boundingBox }) => boundingBox.y1 - boundingBox.y0)
    .filter((height) => height > 0)
    .sort((left, right) => left - right)
  const medianHeight = heights[Math.floor(heights.length / 2)] ?? 1
  const pageBottom = Math.max(...orderedLines.map(({ boundingBox }) => boundingBox.y1), 1)

  const candidates = orderedLines
    .map((line, index) => {
      const candidate = classifyLine(line.text, index)
      if (!candidate) return null
      return {
        ...candidate,
        score: candidate.score + spatialScore(line, medianHeight, pageBottom),
      }
    })
    .filter((line): line is CandidateLine => line !== null)
...
function selectProductName(candidates: readonly CandidateLine[]): string | null {
...
function spatialScore(line: OcrLine, medianHeight: number, pageBottom: number): number {
  const height = Math.max(0, line.boundingBox.y1 - line.boundingBox.y0)
  const topRatio = Math.max(0, Math.min(1, line.boundingBox.y0 / pageBottom))
  const topBonus = (1 - topRatio) * 4
  const sizeBonus = Math.min(6, Math.max(0, height / medianHeight - 1) * 4)
  const confidence = line.confidence
  const confidenceSignal = confidence === undefined
    ? 0
    : confidence < 30 ? -4 : Math.min(2.5, Math.max(0, (confidence - 50) / 20))
  return topBonus + sizeBonus + confidenceSignal
}
