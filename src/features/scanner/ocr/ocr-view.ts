...
export async function openOcrResult(
  ...
  const updateProgress = (progress: OcrProgress): void => {
    ...
    progressLabel.textContent = `Lendo etiqueta...${percentage}`
  }

  const showCandidates = (ocrResult: OcrResult): void => {
    rawText = ocrResult.text
    candidates = extractPriceCandidates(rawText)
    nameInput.value = extractProductName(rawText) ?? ''
  ...
  addButton.addEventListener('click', () => {
    ...
    if (!name) {
      ...
      nameError.textContent = 'Informe o nome do produto.'
      nameInput.focus()
      return
    }
    ...
    submitting = true
    addButton.disabled = true
    finish({ action: 'add', name, unitPriceCents: valueCents })
  })
