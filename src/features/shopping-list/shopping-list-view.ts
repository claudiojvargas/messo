...
import { openCamera } from '../scanner/camera-view'
import { storeCapturedImage } from '../scanner/captured-image'
...
export function mountShoppingList(container: HTMLElement): void {
  ...
  cameraButton.addEventListener('click', async () => {
    ...
    try {
      const image = await openCamera(cameraButton)
      if (image) {
