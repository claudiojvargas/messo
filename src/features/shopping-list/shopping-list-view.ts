...
function productTemplate(product: Product, isEditing: boolean): string {
  ...
  if (isEditing) {
    return `
      <li class="rounded-2xl border border-brand-700/30 bg-white p-4 shadow-sm" data-product-id="${id}">
        <form data-edit-form="${id}" novalidate>
          ...
          <div class="mt-2 flex gap-2"><button class="action-button flex-1 bg-brand-700 text-white" type="submit">Salvar</button><button class="action-button flex-1 border border-stone-200 text-stone-600" type="button" data-action="cancel-edit">Cancelar</button></div>
        </form>
      </li>`
  }

  return `
    <li class="rounded-2xl border border-stone-200 bg-white p-4 shadow-sm" data-product-id="${id}">
      <div class="flex items-start justify-between gap-3">
        <div class="min-w-0"><h3 class="break-words font-bold text-stone-900">${escapeHtml(product.name)}</h3><p class="mt-1 text-sm text-stone-500">${formatCurrency(product.unitPriceCents)} cada</p></div>
      ...
      <div class="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-stone-100 pt-3">
        <div class="flex items-center rounded-xl bg-stone-100 p-1" aria-label="Quantidade de ${escapeHtml(product.name)}">
        ...
        <div class="flex gap-1">
          <button class="action-button text-brand-700" type="button" data-action="edit" aria-label="Editar ${escapeHtml(product.name)}">Editar</button>
          <button class="action-button text-red-700" type="button" data-action="remove" aria-label="Remover ${escapeHtml(product.name)}">Remover</button>
        </div>
      </div>
    </li>`
}
...
function emptyStateTemplate(): string {
  return `<li class="flex min-h-56 flex-col items-center justify-center rounded-3xl border border-dashed border-stone-300 bg-white/70 px-6 py-10 text-center"><span class="mb-4 grid size-14 place-items-center rounded-2xl bg-brand-50 text-brand-700" aria-hidden="true"><svg class="size-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><path d="M6 7h12l-1 13H7L6 7Z" stroke-linejoin="round"/><path d="M9 9V5a3 3 0 0 1 6 0v4" stroke-linecap="round"/></svg></span><h3 class="font-bold text-stone-800">Nenhum produto adicionado ainda</h3><p class="mt-2 max-w-xs text-sm leading-6 text-stone-500">Use os campos acima para começar a organizar sua compra.</p></li>`
}

function pageTemplate(): string {
  return `
    <div class="min-h-dvh bg-stone-50 text-stone-900">
      <header class="bg-brand-800 text-white"><div class="mx-auto flex max-w-3xl items-center justify-between px-5 pb-7 pt-[max(1.25rem,env(safe-area-inset-top))] sm:px-8"><a class="flex items-center gap-3 rounded-lg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-white" href="#main-content" aria-label="Messo, ir para o conteúdo principal"><span class="grid size-10 place-items-center rounded-xl bg-white/12 shadow-inner ring-1 ring-white/15" aria-hidden="true"><svg class="size-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M5 19V8.8A2.8 2.8 0 0 1 7.8 6h8.4A2.8 2.8 0 0 1 19 8.8V19" stroke-linecap="round"/><path d="M9 8V5.5a3 3 0 0 1 6 0V8M8 12h8M8 15.5h5" stroke-linecap="round"/></svg></span><span><span class="block text-2xl font-bold leading-none tracking-tight">messo</span><span class="mt-1 block text-[0.68rem] font-medium uppercase tracking-[0.19em] text-emerald-100/80">compras mais leves</span></span></a><span class="rounded-full bg-white/10 px-3 py-1.5 text-xs font-semibold text-emerald-50 ring-1 ring-white/10">Minha compra</span></div></header>
      <main id="main-content" class="mx-auto max-w-3xl px-4 pb-44 sm:px-8">
        <section class="relative -mt-3 rounded-3xl bg-white p-5 shadow-card ring-1 ring-stone-200/60 sm:p-7" aria-labelledby="add-product-title"><div class="mb-5"><p class="mb-1 text-xs font-bold uppercase tracking-[0.15em] text-brand-700">Novo item</p><h1 id="add-product-title" class="text-xl font-bold tracking-tight">O que vai para o carrinho?</h1></div>
